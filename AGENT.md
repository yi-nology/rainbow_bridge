...
2. **API 接口实现**
   ...
   - `GET /api/v1/pipeline/list?environment_key={key}` - 获取流水线列表（按环境）
...
- 所有配置（系统配置、业务配置）都必须同时指定环境和流水线
- 流水线 API 必须传递 `environment_key` 参数实现按环境过滤
